...
do
    docker rm -f 1mclient_$c
    docker run -v $(pwd)/client:/client --name 1mclient_$c -d docker.oa.com/public/tlinux2.2_base /client \
    -conn=${CONNECTIONS} -ip=${IP} -sm "${DATE}" -c ${CONCURRENCY}
done